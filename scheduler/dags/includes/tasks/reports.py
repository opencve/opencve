...
@task
def generate_report_summaries(**context):
    """
    ...
    # LLM system prompt
    prompt_path = (
        pathlib.Path(__file__).parent.parent / "data" / "cve_report_analyser.prompt"
    )
    ...
    for report in reports:
        ...
        report_cves_data = [build_cve_data_for_llm(cve) for cve in report_cves]
        llm_payload = {
            "cves_count": report_cves_count,
            "cves_score": {
                r["score"]: r["count"] for r in report_cves_score_distribution
            },
            "cves_data": report_cves_data,
        }
