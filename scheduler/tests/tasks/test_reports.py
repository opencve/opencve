...
def test_generate_report_summaries_missing_api_key():
    """Test generate_report_summaries with missing LLM API key"""
    mock_context = {
    ...
    with patch(
        ...
        with pytest.raises(AirflowSkipException, match="LLM API key is not configured"):
            generate_report_summaries.function(**mock_context)


def test_generate_report_summaries_missing_api_url():
    """Test generate_report_summaries with missing LLM API URL"""
    mock_context = {
        "data_interval_start": pendulum.datetime(2025, 1, 1, 2, 0, tz="UTC")
    }
...
def test_generate_report_summaries_no_reports():
    """Test generate_report_summaries with no reports to process"""
    mock_context = {
        "data_interval_start": pendulum.datetime(2025, 1, 1, 2, 0, tz="UTC")
    }
    ...
    with patch("includes.tasks.reports.conf.get", side_effect=mock_get):
        with patch("includes.tasks.reports.PostgresHook") as mock_hook:
            ...
            result = generate_report_summaries.function(**mock_context)
            assert result is None


def test_generate_report_summaries_success(tests_path, tmp_path_factory):
    """Test successful generate_report_summaries execution"""
    ...
    with (
        ...
        generate_report_summaries.function(**context)
