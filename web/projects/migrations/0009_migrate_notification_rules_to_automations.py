...
# Each legacy notification type maps to one or more automation trigger identifiers.
TYPE_TO_TRIGGERS = {
...
def _build_conditions(cvss31):
    # Builds the condition tree from legacy metrics.cvss31 using
    # the OR(AND(...)) shape the DSL expects.
    and_children = []
...
def _build_triggers(types):
    # Collects unique trigger ids for the given legacy type list.
    triggers = []
    for item in types:
