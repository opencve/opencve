...
from opencve.api.v2.scopes import APIScope, token_has_scope


class IsOrganizationTokenAuthenticated(permissions.BasePermission):
    def has_permission(self, request, view):
        return bool(getattr(request, "organization_token", None))


class RequiresScope(permissions.BasePermission):
    """Permission to check if the token has the required scope"""
